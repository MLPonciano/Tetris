...
function drawNext() {
    ...
    for (let r=0; r < 4; r++) {
        for (let c=0; c < 4; c++) {
            ...
            if (shape[r] && shape[r][c]) cell.style.background=nextPiece.color;
...
function drawHold() {
    ...
    for (let r=0; r < 4; r++) {
        for (let c=0; c < 4; c++) {
            const cell=document.createElement("div");
            cell.style.background="";
            if (shape[r] && shape[r][c]) cell.style.background=holdPiece.color;
...
function disableControls() {
    document .querySelectorAll(".dpad button, .ab-buttons button") .forEach((btn)=> (btn.disabled=true));
}

function enableControls() {
    document .querySelectorAll(".dpad button, .ab-buttons button") .forEach((btn)=> (btn.disabled=false));
}
